  ...
  const booleanValue = useSelector((state: RootState) => state.boolean.value);
    ...
    return (
        <div className={`flex w-full items-center justify-center z-50 top-5 fixed transition-transform duration-300 h-[60px] ${show ? 'translate-y-0' : '-translate-y-full'}`}>
            <nav className={`flex px-8 py-3 inset-x-0 bg-[#211F23]/40 shadow backdrop-blur-lg md:top-6 rounded-3xl items-center ${booleanValue ? "text-black" : "text-white"}  gap-5`}>
